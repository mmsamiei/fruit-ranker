...
items = {}
pairwise_results = defaultdict(lambda: defaultdict(lambda: [0, 0]))  # [wins, total]
image_folder = "images"  # Replace with your image folder path

# Load images and initialize ratings
...
def update_ratings(winner, loser):
    ...
    items[loser]["plays"] += 1
    pairwise_results[winner][loser][0] += 1
    pairwise_results[winner][loser][1] += 1
    pairwise_results[loser][winner][1] += 1
...
def choose_item(choice):
    ...
    return (
        items[current_items[0]]["image"],
        items[current_items[1]]["image"],
        current_items[0],
        current_items[1],
        get_ratings_table(),
...
def get_pairwise_heatmap():
    ...
    for i, item1 in enumerate(item_names):
        for j, item2 in enumerate(item_names):
            if i != j:
                wins, total = pairwise_results[item1][item2]
...
# Initialize with two random items
current_items = get_random_items()
...
with gr.Blocks() as demo:
    ...
    with gr.Tabs():
        with gr.TabItem("Compare Items"):
            ...
            with gr.Row():
                img1 = gr.Image(value=items[current_items[0]]["image"], label="Item 1")
                img2 = gr.Image(value=items[current_items[1]]["image"], label="Item 2")
